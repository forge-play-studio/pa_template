...
function validateSceneMarker(value, path, add) {
  ...
  if (value.metadata != null && !isObject(value.metadata)) add(`${path}.metadata`, 'marker.metadata must be an object when present');
}
...
function validateSceneMarkerTarget(value, path, add) {
  ...
  if (value.metadata != null && !isObject(value.metadata)) add(`${path}.metadata`, 'marker target metadata must be an object when present');
}
...
function validateSceneMarkerFrame(value, path, add) {
  ...
  if (!isObject(value.origin)) add(`${path}.origin`, 'marker semanticFrame.origin must be a vector object');
...
function validateSceneMarkerGeometry(value, path, add) {
  ...
  if (value.kind === 'polyhedron') {
    ...
    if (!Array.isArray(value.vertices)) add(`${path}.vertices`, 'polyhedron marker vertices must be an array');
      ...
      if (!isObject(entry)) add(vertexPath, 'polyhedron marker vertex must be a vector object');
