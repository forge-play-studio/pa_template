...
build_variant() {
  ...
  if LOCALE="$locale" CHANNEL="$channel" TRACKING="$tracking" BUILD_MATRIX=true npx tsc && LOCALE="$locale" CHANNEL="$channel" TRACKING="$tracking" BUILD_MATRIX=true npx vite build && npm run check:prod-debug; then
    mkdir -p "$out_dir"
