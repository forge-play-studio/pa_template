...
import type { VfxEffectHandle, VfxParamValues } from '../assets/vfx';
...
export class ProjectVfxDirector implements GameplayModule {
  ...
  private resolveMountNode(usage: ProjectVfxUsageConfig): TransformNode | null {
    const node = this.runtimeNodes.getRuntimeNode(usage.positionSource.nodeId);
    if (!node) {
      console.warn(`[ProjectVfxDirector] Missing ${usage.positionSource.kind} node "${usage.positionSource.nodeId}" for usage "${usage.id}".`);
      return null;
    }

    if (usage.positionSource.kind === 'socket' && readSceneMarker(node)?.type !== 'effect-socket') {
      console.warn(`[ProjectVfxDirector] Node "${usage.positionSource.nodeId}" is not an effect-socket.`);
      return null;
    }
