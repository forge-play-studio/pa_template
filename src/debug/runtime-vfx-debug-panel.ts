...
export function mountRuntimeVfxDebugPanel(options: RuntimeVfxDebugPanelOptions): RuntimeVfxDebugPanel {
  ...
  function createBooleanRow(definition: VfxParamDefinition): HTMLElement {
    const row = ownerDocument.createElement('label');
    row.style.cssText = [
      'display:grid',
      'grid-template-columns:92px 1fr',
    ...
    const input = ownerDocument.createElement('input');
    input.type = 'checkbox';
    input.checked = readDraftBoolean(draftValues[definition.key], readDefaultBoolean(getSelectedPackage(), definition.key));
    input.dataset.vfxParamKey = definition.key;
    input.dataset.vfxBooleanParam = 'true';
    input.style.cssText = [
      'width:16px',
      'height:16px',
      'margin:0',
    ...
    input.addEventListener('click', (event) => event.stopPropagation());
    input.addEventListener('change', () => {
      draftValues[definition.key] = input.checked ? 'true' : 'false';
      syncCheckboxText();
      schedulePreview();
    });
...
function readDefaultBoolean(effectPackage: VfxEffectPackage | null, key: string): boolean {
...
function readNumberProp(definition: VfxParamDefinition, key: 'min' | 'max', fallback: number): number {
  const value = definition.kind === 'number'
    || definition.kind === 'opacity'
    || definition.kind === 'lifetime'
