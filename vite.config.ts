...
function validateVfxUsagesDocument(document: any): string[] {
  ...
  document.usages.forEach((usage: any, index: number) => {
    ...
    const positionSource = usage.positionSource;
    if (!positionSource || typeof positionSource !== 'object' || Array.isArray(positionSource)) {
      add(`${base}.positionSource`, 'must be an object');
    } else {
      const kind = typeof positionSource.kind === 'string' ? positionSource.kind : '';
      if (!VFX_USAGE_TARGET_KINDS.has(kind)) add(`${base}.positionSource.kind`, 'must be socket or node');
      if (placement && kind && placement !== kind) add(`${base}.positionSource.kind`, 'must match placement');
      if (typeof positionSource.nodeId !== 'string' || positionSource.nodeId.trim().length === 0) {
        add(`${base}.positionSource.nodeId`, 'must be a non-empty string');
      }
    }
