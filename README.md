...
集中存放项目侧的 `fps-game-editor` 模式接入、feature 配置与 authored-data hooks，例如 `editor-entry.ts` 的 `enterEditorMode / enterPlayMode` 装配，以及 GroundDecal presets、layer policy 和 texture resolver。GameApplication、GameWorld 与 runtime debug 仍由 `src/dev/DevHost.ts` 拥有；Editor Entry 状态机、入口 UI、诊断、Editor Plugin Host、默认 loading 文案和 Babylon 投射由 `@fps-games/editor/playable-sdk` 提供。
...
3. `runtime-debug-bootstrap.ts`：dev-only debug 总入口；由项目 `src/dev/DevHost.ts` 为每个 Play mode lifetime 挂载和清理。
...
5. 面板必须由 `src/dev/DevHost.ts` 的 dev-only composition root 加载，不要在 production-owned 文件里静态 import debug module 的值。
...
5. 面板统一注册到 `src/debug/panel-manifest.ts`，由 `runtime-gameplay-debug-panels.ts` mount，并通过 `src/dev/DevHost.ts` 的 dev-only composition root 加载。
