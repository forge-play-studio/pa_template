...
3. `runtime-debug-bootstrap.ts`：dev-only debug 总入口；只由 `src/dev/dev-entry.ts` / `src/dev/DevHost.ts` 的 dev dynamic import 链路加载，并从 `src/services/fps-game-editor/local-editor.ts` 挂载产品化编辑器 host。
...
5. 面板统一注册到 `src/debug/panel-manifest.ts`，由 `runtime-gameplay-debug-panels.ts` mount，并通过 `src/dev/dev-entry.ts` / `src/dev/DevHost.ts` 的 dev-only dynamic import 加载。
